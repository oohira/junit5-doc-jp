...
| `@Timeout`               | テストやテストファクトリ、テストテンプレート、ライフサイクルメソッドが指定の実行時間を超えると失敗させます。このアノテーションは、_継承_ されます。
...
[WARNING]
.`assertTimeoutPreemptively()` によるプリエンプティブなタイムアウト
====
<<writing-tests-declarative-timeouts, 宣言的なタイムアウト>> とは違って、
...
https://junit.org/junit4/javadoc/latest/org/hamcrest/Matcher.html[`Matcher`]
を受け付ける
...
WARNING: `{EnabledIf}` または `{DisabledIf}` を使った条件付きテスト実行は、
JUnit Jupiter の今後のリリースで削除されるため _非推奨_ です。
...
-- 例えば、`@ValueSource(strings = {"{nbsp}", "{nbsp}{nbsp}{nbsp}", "\t", "\n"})` のようにです。
...
引用符で囲まれた空の値 `''` は `emptyValue` 属性が指定されていない限り空の `String` になりますが、
...
を指定する場合に使えます。`ArgumentsProvider` 実装は、トップレベルのクラスまたは
`static` なネストクラスとして宣言しなければならない点に注意してください。
...
NOTE: 10 進数および 16 進数、8 進数の `String` リテラルは、それぞれの整数型に変換されます:
`byte`, `short`, `int`, `long`, そして対応するラッパー型です。
...
- __ファクトリメソッド__: ターゲット型に宣言された非 private の `static` メソッドで、
  `String` 型の引数を 1 つだけ受け取ってターゲット型のインスタンスを返します。
  メソッドの名前は任意で、特定のルールに従う必要はありません。
- __ファクトリコンストラクタ__: ターゲット型に宣言された非 private のコンストラクタで、
  `String` 型の引数を 1 つだけ受け取ります。ターゲット型は、トップレベルのクラスか
  `static` なネストクラスとして宣言しなければならない点に注意してください。
...
暗黙的な変換を使う代わりに、次の例のように `@ConvertWith` アノテーションを使って仮引数のための
`ArgumentConverter` を明示的に指定することもできます。`ArgumentConverter` 実装は、
トップレベルのクラスか `static` なネストクラスとして宣言しなければならない点に注意してください。
