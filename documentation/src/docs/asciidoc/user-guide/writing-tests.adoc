...
を少なくとも1つは宣言し、その引数をテストメソッドで _消費 (consume)_ しなければいけません。

以下は、引数のソースとして `String` 配列を指定するために `@ValueSource`
アノテーションを使用したパラメーター化テストの例を示しています。
...
`@MethodSource` は、テストクラスまたは外部のクラスの _ファクトリ_ メソッドによる指定を可能にします。
...
JUnit Jupiterは、`@ParameterizedTest` に与えられた引数に対して、
https://docs.oracle.com/javase/specs/jls/se8/html/jls-5.html#jls-5.1.2[プリミティブ型の
拡大変換] をサポートしています。例えば、`@ValueSource(ints = { 1, 2, 3 })`
アノテーションが付与されたパラメーター化テストは、 `int` 型の引数だけでなく
`long` や `float`、`double` 型の引数でも宣言することができます。
...
`Stream`, `Collection`, `Iterable`, `Iterator` いずれかを
