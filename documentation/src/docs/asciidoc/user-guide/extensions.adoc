...
フレームワークは単にテストクラスのデフォルトコンストラクタを実行して初期化します。

`TestInstanceFactory` を実装する拡張機能は、トップレベルのクラスだけでなく `@Nested`
テストクラスに対しても登録できます。`@Nested` テストクラスに対して登録されたファクトリは、
その内部クラスに登録されるほかのファクトリを上書きします。

[WARNING]
====
`TestInstanceFactory` を実装する複数の拡張機能を1つのクラスに登録すると、
そのクラスおよびネストするクラスのすべてのテストで例外がスローされます。
親クラスに登録された `TestInstanceFactory` は _継承される_ 点に注意してください。
