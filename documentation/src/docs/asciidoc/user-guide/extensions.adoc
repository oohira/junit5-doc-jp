...
JUnit 4の `Runner` や `TestRule`, `MethodRule` などの拡張ポイントとは異なり、
...
例えば、`MyFirstTests` と `MySecondTests` のテスト実行は、`DatabaseExtension` と
`WebServerExtension` によって **まさにこの順序で** 拡張されます。
====
...
_テストクラス_ のコンストラクタや _テストメソッド_, _ライフサイクルメソッド_
(<<writing-tests-classes-and-methods>> 参照) が引数をもつ場合、
...
[source,java,indent=0]
.テスト実行中の IOException 例外をハンドリングする拡張機能
----
...
"`apple`" と "`banana`" になります。各実行では、メソッド引数を解決するために使われるカスタムの
...
ライフサイクルメソッドに加えて、多くの拡張機能コールバックが呼び出されます。
...
次の図は、ユーザーコードと拡張機能コードの相対的な実行順序を示しています。
...
次の表は、<<extensions-execution-order-diagram>> の図の 16 ステップをさらに説明しています。
...
最も単純な場合は、実際のテストメソッドだけが実行されます (ステップ 8)。
