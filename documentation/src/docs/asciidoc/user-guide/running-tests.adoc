...
NOTE: https://issues.apache.org/jira/browse/SUREFIRE-1330[バージョン 2.22.0] から、
...
JUnitチームは、Maven Surefire 2.22.0より古いバージョンで `mvn test` でJUnit 4と
JUnit Jupiterのテストを実行できるMaven Surefireプロバイダを開発してきました。しかし、
Surefire 2.22.0がリリースされたため、JUnitチームによる `junit-platform-surefire-provider`
は非推奨となり、JUnit Platformの今後のリリースで廃止されます。

NOTE: `junit-platform-surefire-provider` とはMaven Surefire {surefire-version}
を使ってください。
