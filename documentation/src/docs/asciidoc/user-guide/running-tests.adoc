...
もし先に述べたもの以外のエディタやIDEを使用している場合、JUnitチームは
JUnit 5の利用を補助するための代替案を2つ提供します。
<<running-tests-console-launcher>> を（例えばコマンドラインから）手動で使うか、
もしIDEがJUnit 4に組み込み対応しているなら
<<running-tests-junit-platform-runner,JUnit 4ベースのランナー>> でテストを実行するかです。
...
に対する `testImplementation` 依存と、JUnit Jupiter `TestEngine`
実装に対する `testRuntimeOnly` 依存を次のように指定する必要があります。
...
そのためには、JUnit 4に対する `testImplementation` 依存と、JUnit Vintage
`TestEngine` 実装に対する `testRuntimeOnly` 依存が必要です。
...
[WARNING]
.JUnit Platform Maven Surefireプロバイダは廃止
====
JUnitチームによって開発されていた `junit-platform-surefire-provider` は、
JUnit Platform 1.3で非推奨となり、JUnit Platform 1.4で廃止されました。
代わりに、Maven Surefireのネイティブサポートを利用してください。
====

https://issues.apache.org/jira/browse/SUREFIRE-1330[バージョン 2.22.0] から、
Maven SurefireとMaven Failsafeは、JUnit Platform上でテストを実行するための
https://maven.apache.org/surefire/maven-surefire-plugin/examples/junit-platform.html[ネイティブサポート]
を提供しています。 `{junit5-jupiter-starter-maven}` プロジェクトの `pom.xml`
ファイルがMaven Surefireプラグインの使い方を示すとともに、Mavenビルドを設定するための出発点となるでしょう。

[[running-tests-build-maven-engines-configure]]
===== テストエンジンを設定する

Maven SurefireまたはMaven Failsafeにテストを実行させるためには、
`TestEngine` の実装が少なくとも1つ実行時のクラスパスに必要です。
...
Maven SurefireとMaven Failsafeは、JUnit 4ベースのテストも実行することができます。
...
Ant のバージョン `1.10.6` から、`junitlauncher` タスクは
link:https://ant.apache.org/manual/Tasks/junitlauncher.html#fork[異なる JVM に fork してのテスト実行] をサポートしています。
...
以下の例のテストメソッドには、`org.junit.Test` (JUnit 4) ではなく、
`org.junit.jupiter.api.Test` (JUnit Jupiter) アノテーションが
付与されていることに注意してください。さらに、この場合はテストクラスを
`public` にしなければならない点にも注意してください。そうしないと、
一部のIDEやビルドツールではJUnit 4のテストクラスとして認識されません。
...
`JUnitPlatformSuiteDemo` は、`example` パッケージとそのサブパッケージから
...
(例えば、 *product*, *catalog*, *shipping*) のタグを付与するとき、次のようなタグ式が使えます。
...
| +catalog \| shipping+
| *catalog* または *shipping* に対するすべてのテスト

| +catalog & shipping+
| *catalog* かつ *shipping* に対するすべてのテスト

| +product & !end-to-end+
| *product* に対するテストのうち、_end-to-end_ レベル以外のすべてのテスト

| +(micro \| integration) & (product \| shipping)+
| *product* または *shipping* に対するテストのうち、_micro_ または _integration_ レベルのすべてのテスト
|===
